...
const captureSelectKeyspaceAndTable = `(?is)SELECT.*FROM\s+([A-Za-z_0-9.]+)`
const captureSelectReturnValues = `(?is)SELECT(.*)FROM`
const captureSelectSingleInputValues = `(?i)([A-Za-z_0-9()]+)\s*[=><]+\s*\?`
const captureSelectSliceInputValues = `(?i)([A-Za-z_0-9()]+)\s+IN\s*\(%s`

func loadSelectStatement(stmt string, ddlConfig ddl.Config, out *StatementConfig) error {
	list := util.NewErrorList()

	keyspaceAndTableNamesBlock, err := util.GetFirstMatch(captureSelectKeyspaceAndTable, stmt)
	if err != nil {
	...
	singleInputValues, _ := util.GetAllMatches(captureSelectSingleInputValues, stmt)
	...
	sliceInputValues, _ := util.GetAllMatches(captureSelectSliceInputValues, stmt)
	...
	outputValuesBlock, _ := util.GetAllMatches(captureSelectReturnValues, stmt)
